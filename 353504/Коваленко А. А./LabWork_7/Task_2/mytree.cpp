...
double MyTree::findAverageKey() {
    ...
    int MyTree::findClosestToAverage() {
        double average = findAverageKey();
        TreeNode* closestNode = nullptr;
        double closestDistance = std::numeric_limits<double>::max();
        findClosestNode(root, average, closestNode, closestDistance);
        return closestNode->key;
    }
void MyTree::findSumAndCount(TreeNode* node, int& sum, int& count) {
        if (node == nullptr) {
            return;
        }
        sum += node->key;
        count++;
        findSumAndCount(node->left_child, sum, count);
        findSumAndCount(node->right_child, sum, count);
    }

    void MyTree::findClosestNode(TreeNode* node, double average, TreeNode*& closestNode, double& closestDistance) {
        if (node == nullptr) {
            return;
        }
        double distance = std::abs(node->key - average);
        if (distance < closestDistance) {
            closestDistance = distance;
            closestNode = node;
        }
        findClosestNode(node->left_child, average, closestNode, closestDistance);
        findClosestNode(node->right_child, average, closestNode, closestDistance);
    }
